...
export default function CentralDashboard() {
  ...
  return (
    <div className="p-4 md:p-8 lg:p-10 w-full space-y-10 pb-16">
      {/* Welcome Header */}
      <div>
            ...
            <div className="p-5 md:p-8 border-b border-white/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs md:text-sm text-gray-500">Pending offers</p>
                </div>
              </div>
              <button className="w-full sm:w-auto px-6 py-2.5 rounded-full bg-[#00A5E5] text-white text-sm font-semibold transition-all shadow-[0_0_20px_rgba(124,92,255,0.2)] hover:shadow-[0_0_30px_rgba(124,92,255,0.4)]">
                View All Offers
              </button>
            </div>
            ...
            <div className="p-4">
              {data?.incoming_offers?.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 w-full md:w-auto justify-between md:justify-end mt-2 md:mt-0">
                        ...
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                          <button className="flex-1 sm:flex-none px-5 py-2 rounded-xl bg-[#00A5E5] text-white text-sm font-bold transition-all">
                            Accept
                          </button>
                ...
                <div className="flex flex-col items-center justify-center py-16">
                  <p className="text-xl font-semibold text-gray-400">Data not found</p>
                  <p className="text-sm text-gray-500 mt-2">No incoming offers available.</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="space-y-4">
              {data?.upcoming_bookings?.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-10">
                  <p className="text-lg font-semibold text-gray-400">Data not found</p>
                  <p className="text-sm text-gray-500">No upcoming bookings available.</p>
                </div>
              )}
            </div>
          </div>
                ...
                data.recent_activities.map((activity: any, index: number) => (
                  <div key={activity.id} className="flex gap-4 group">
                    ...
                    <div className="space-y-1">
                      <h4 className="text-sm font-bold text-gray-200">{activity.summary}</h4>
                      <p className="text-xs text-gray-500 font-medium">{activity.detail}</p>
